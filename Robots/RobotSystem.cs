...
namespace Robots
{
    public enum Manufacturers { ABB, KUKA, UR, FANUC, Staubli, Aubo, ESTUN, Other, All };
    ...
    public abstract class RobotSystem
    {
        ...
        public static List<string> ListTrackSystems()
        {
            var names = new List<string>();
            var elements = new List<XElement>();
            //string folder = $@"{AssemblyDirectory}\robots";
        ...
        public static RobotSystem Load(string name, Plane basePlane, string trackName = null, string revolverName = null)
        {
            ...
            if (Directory.Exists(folder))
            {
                ...
                if (hasTrack)
                {
                    element.Element("Mechanisms").Element("RobotArm").AddAfterSelf(trackElement);
                }
        ...
        private static RobotSystem Create(XElement element, Plane basePlane)
        {
            ...
            if (type == "RobotCell")
            {
                ...
                switch (manufacturer)
                {
                    ...
                    //    return new RobotCellAubo(name, mechanicalGroups, io, basePlane, environment);
                    //case (Manufacturers.ESTUN):
                    //    return new RobotCellEstun(name, mechanicalGroups, io, basePlane, environment);
