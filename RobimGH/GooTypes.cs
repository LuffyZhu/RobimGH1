...
namespace Robim.Grasshopper
{
    ...
    public class GH_Zone : GH_Goo<Zone>
    {
        ...
        public override bool CastFrom(object source)
        {
            ...
            if (source is GH_Number)
            {
                Value = new Zone("DIS",(source as GH_Number).Value);
                return true;
            }
            ...
            {
                string[] texts = (source as GH_String).Value.Split(',');
                double[] values = new double[texts.Length - 1];
                ...
                if (texts.Length == 2)
                {
                    Value = new Zone(texts.Last(), values[0]);
                    return true;
                }
                else if (texts.Length == 3)
                {
                    Value = new Zone(texts.Last(), values[0], values[1]);
                    return true;
                }
            }
    ...
    public class GH_PR : GH_Goo<ProcessRegister>
    {
        public GH_PR() { this.Value = ProcessRegister.Default; }
        public GH_PR(GH_PR goo) { this.Value = goo.Value; }
        public GH_PR(ProcessRegister native) { this.Value = native; }
        public override IGH_Goo Duplicate() => new GH_PR(this);
        public override bool IsValid => true;
        public override string TypeName => "PR";
        public override string TypeDescription => "Processor Register";
        public override string ToString() => this.Value?.ToString();
        public override object ScriptVariable() => Value;
        ...
        public override bool CastFrom(object source)
        {
            if (source is ProcessRegister)
            {
                Value = source as ProcessRegister;
                return true;
            }

            if (source is GH_Number)
            {
                string texts = (source as GH_String).Value;
                string intemp = texts.Split(',')[0];
                //double[] values = new double[texts.Length];
                int index = Convert.ToInt32(intemp);
                Value = new ProcessRegister(index, texts);
                return true;
            }
            ...
            //{
            //    string text = (source as GH_String).Value;
            //    string[] prcontent = text.Split(',');

            //    var cc = new double[prcontent.Length];
            //    for (int i = 0; i < prcontent.Length; i++)
            //        if (!GH_Convert.ToDouble_Secondary(prcontent[i], ref cc[i])) return false;

            //    Value = new ProcessRegister(cc);
            //    return true;
            //}
            return false;
        }
    ...
    public class GH_TargetCurve : GH_Goo<TargetCurve>
    {
        public GH_TargetCurve() { this.Value = TargetCurve.Default; }
        public GH_TargetCurve(GH_TargetCurve goo) { this.Value = goo.Value; }
        public GH_TargetCurve(TargetCurve native) { this.Value = native; }
        public override IGH_Goo Duplicate() => new GH_TargetCurve(this);
        public override bool IsValid => true;
        public override string TypeName => "TargetCurve";
        public override string TypeDescription => "TargetCurve";
        public override string ToString() => this.Value?.ToString();
        public override object ScriptVariable() => Value;

        public override bool CastFrom(object source)
        {
            if (source is TargetCurve)
            {
                Value = source as TargetCurve;
                return true;
            }
            return false;
        }
