...
namespace Robim
{
    public class CheckProgram
    {
        ...
        int CellTargetCount;
        public static CancellationTokenSource tokenSource { get; internal set; }

        public delegate void StepCountHandler(int answer);
        public event StepCountHandler StepChange;

        int i = -1;
        Stopwatch stopwatch = new Stopwatch();
        protected virtual void StepcountChange(int one)
        {
            int step = one * 100 / CellTargetCount;
            if(step != i)
            {
                i = step;
                stopwatch.Stop();
        ...
        internal CheckProgram(Program program, List<CellTarget> cellTargets, double stepSize)
        {
            ...
            this.CellTargetCount = cellTargets.Count;
        ...
        public void Compute()
        {
            ...
            Task task = Task.Factory.StartNew(() =>
            {
                ...
                //this.indexError = FixTargetMotionsMuiltCompute(cellTargets, stepSize);
            }, token);
            task.Wait();
        }
        ...
        void FixTargetAttributes(List<CellTarget> cellTargets)
        {
            ...
            #region New_ForLoop
            for (int count = 0; count < cellTargets.Count; count++)
            {
                var cellTarget = cellTargets[count];
                for(int count2 = 0; count2 < cellTarget.ProgramTargets.Count; count2++)
                {
                    var programTarget = cellTarget.ProgramTargets[count2];
                    int externalCount = 0;
            ...
            // Name attributes with no name
            {
                ...
                foreach (var attribute in program.Attributes.ToList())
                {
                    if (attribute.Name == null)
                    {
                        ...
                        if (type == typeof(Zone))
                        {
                            var zone = attribute as Zone;
                            if(zone.Type == "VEL")
                            {
                                name = $"{type.Name}_Speed{i - 1:000}";
                            }
                            else
                            {
                                name = $"{type.Name}_Distance{i - 1:000}";
                            }
                        }
                        else
                        {
                            name = $"{type.Name}{i - 1:000}";
                        }
        ...
        int FixTargetMotions(List<CellTarget> cellTargets, double stepSize)
        {
            ...
            for (int i = 0; i < cellTargets.Count; i++)
            {
                ...
                if (i == 0)
                {
                    var firstKinematics = robotSystem.Kinematics(cellTarget.ProgramTargets.Select(x => x.Target));
                    cellTarget.SetTargetKinematics(firstKinematics, program.Errors, program.Warnings);
                ...
                else
                {
                    ...
                    // no interpolation
                    {
                        ...
                        {
                            if (kineTargets[j] is CartesianTarget target && target.Motion != Motions.Joint)
                            {
                    ...
                    foreach (var target in linearTargets)
                    {
                        ...
                        double distance = prevPlane.Origin.DistanceTo(target.Plane.Origin);//应该使用curve length 给arc
                    ...
                    for (double j = 1; j <= divisions; j++)
                    {
                        ...
                        var interTarget = cellTarget.ShallowClone();//固定
                        var kineTargets = cellTarget.Lerp(prevTarget, robotSystem, t, 0.0, 1.0);//变量
                        var kinematics = program.RobotSystem.Kinematics(kineTargets, prevJoints);//变量
                        ...
                        for (int num = 0; num < interTarget.ProgramTargets.Count; num++)
                        {
                            var target = interTarget.ProgramTargets[0];
        ...
        int FixTargetMotionsMuiltCompute(List<CellTarget> cellTargets, double stepSize)
        {
            double time = 0;
            List<CellTarget> celltargets_split = new List<CellTarget>();

            //需要加入时间才能进行模拟
            StepcountChange(0);

            int errorIndex = -1;
            var cellTarget = cellTargets[0];
            Mechanism mechanism = null;

            var firstKinematics = robotSystem.Kinematics(cellTarget.ProgramTargets.Select(x => x.Target));//姿态
            mechanism = firstKinematics[0].Mechanism;
            cellTarget.SetTargetKinematics(firstKinematics, program.Errors, program.Warnings);//设定
            CheckUndefined(cellTarget, cellTargets);//check
            keyframes.Add(cellTarget.ShallowClone());//?
            if (program.Errors.Count > 0) { errorIndex = 0; }//错误的target

            var prevTarget = cellTargets[0];
            var prevJoints = prevTarget.ProgramTargets.Select(x => x.Kinematics.Joints);

            #region 分段:将所有targets再依步长分段
            for (int index0 = 1; index0 < cellTargets.Count; index0++)
            {
                prevTarget = cellTargets[index0 - 1];
                cellTarget = cellTargets[index0];
                double divisions = 1;
                var linearTargets = cellTarget.ProgramTargets.Where(x => !x.IsJointMotion);
                //   if (linearTargets.Count() > 0) program.Errors.Clear();
                ...
                {
                    var prevPlane = target.GetPrevPlane(prevTarget.ProgramTargets[target.Group]);
                    double distance = prevPlane.Origin.DistanceTo(target.Plane.Origin);//应该使用curve length 给arc//target plane.origin
                    double divisionsTemp = Ceiling(distance / stepSize);
                    if (divisionsTemp > divisions) divisions = divisionsTemp;
                }
            ...
            #endregion
            //AllKinematics allKinematics = new AllKinematics(, mechanism, prevJoints.FirstOrDefault());
            ...
            //        CheckUndefined(cellTarget, cellTargets);//check
            //        keyframes.Add(cellTarget.ShallowClone());//?
            //        if (program.Errors.Count > 0) { errorIndex = i; }//错误的target
            //    }
            //    else
            //    {
            //        var prevTarget = cellTargets[i - 1];
        ...
        Tuple<double, double, int> GetSpeeds(ProgramTarget target, ProgramTarget prevTarget)
        {
            ...
            for (int i = 0; i < (target.Kinematics.Joints.Length - target.Target.External.Length); i++)
            {
