...
namespace Robim
{
    public static class Util
    {
        ...
        internal static string LibraryPath
        {
            get
            {
                return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Robim");
            }
        }
        ...
        public static List<double> CalculatePlatformAngle(Vector3d vectorA, List<Vector3d> vectorB, Plane plane, Boolean rev)
        {
            ...
            for (int i = 0; i < vectorB.Count; i++)
            {
                ...
                if (i == 0)
                {
                    double a = Rhino.Geometry.Vector3d.VectorAngle(vectorA, vectorB[i], plane);
                    if (a > PI)
                    {
                        a = a - 2 * PI;
                        angle.Add(a);
                ...
                {
                    double a = Rhino.Geometry.Vector3d.VectorAngle(vectorB[i - 1], vectorB[i], plane);
                    if (a > PI)
                    {
                        a = a - 2 * PI;
                        a = angle[i - 1] + a;
    ...
    public class RobimGeneric<T>
    {
        public List<T> GenericList { get; internal set; }
        ...
        {
            int length = GenericList.Count;
            Type type = GenericList[0].GetType();
            string typename = type.BaseType.GenericTypeArguments[0].Name;
            return $"<{Name}> : Group of {length} {typename}";
        }
    }
}