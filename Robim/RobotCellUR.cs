...
namespace Robim
{
    public class RobotCellUR : RobotCell
    {
        public RobotUR Robot { get; }
        //public RemoteConnection Remote { get; } = new RemoteConnection();
        // public URRealTime URRealTime { get; set; }

        internal RobotCellUR(string name, List<MechanicalGroup> mechanicalGroups, IO io, Plane basePlane, Mesh environment, RobimFormSystem robimFormSystem) : base(name, Manufacturers.UR, mechanicalGroups, io, basePlane, environment,robimFormSystem)
        {
            /*Remote = new RemoteUR();
            this.Robot = mechanicalGroups[0].Robot as RobotUR;
            this.DisplayMesh = new Mesh();
            DisplayMesh.Append(mechanicalGroups[0].Robot.DisplayMesh);
