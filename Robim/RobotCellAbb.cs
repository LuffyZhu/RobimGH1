...
namespace Robim
{
    ...
    {
        internal RobotCellAbb(string name, List<MechanicalGroup> mechanicalGroups, IO io, Plane basePlane, Mesh environment,RobimFormSystem robimFormSystem) : base(name, Manufacturers.ABB, mechanicalGroups, io, basePlane, environment, robimFormSystem)
        {
    ...
    ";
        ...
        class RapidPostProcessor
        {
            ...
            List<string> MainModule(int group)
            {
                ...
                {
                    for (int i = 0; i < cell.MechanicalGroups[group].Externals.Count; i++)
                    {
                        string externalname = cell.MechanicalGroups[group].Externals[i].Model.Remove(0, 4);//Remove "ABB."
                        #region 配合嘉定工厂变位机
                        if (externalname.Contains("/"))
                        {
                            string[] names = externalname.Split('/');
                            code.Add($"ActUnit {names[0].Split('_').Last()};");
                            code.Add($"ActUnit {names[1].Split('_').Last()};");
                        }
                        #endregion
                        else
                            code.Add($"ActUnit {externalname};");
                        //code.Add($"ActUnit M7DM1");//嘉定的导轨名称
                    }
                }
            ...
            List<string> SubModule(int file, int group)
            {
                ...
                for (int j = start; j < end; j++)
                {
                    ...
                    if (programTarget.IsJointTarget)
                    {
                        ...
                        double[] joints = jointTarget.Joints.Take(6).ToArray();//只提取Robot的Joints 不然会报错
