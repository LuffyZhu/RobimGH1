...
namespace Robim
{
    internal class RobotCellCobot : RobotCell
    {
        public RobotCobot Robot { get; }

        internal RobotCellCobot(string name, List<MechanicalGroup> mechanicalGroups, IO io, Plane basePlane, Mesh environment, RobimFormSystem robimFormSystem) : base(name, Manufacturers.Cobot, mechanicalGroups, io, basePlane, environment, robimFormSystem)
        {
        ...
        internal override List<List<List<string>>> Code(Program program) => new CobotScriptPostProcessor(this, program).Code;
        ...
        internal override void SaveCode(Program program, string folder)
        {
            ...
            //var finalCode = joinedCode + "\r\n" + $"{program.Name}()";//ÔÚÄ£ÄâÀï¼ÓÉÏÕâ¸öÁË
            File.WriteAllText(file, joinedCode);
        }



        class CobotScriptPostProcessor
        {
            //RobotUR robot;
            RobotCellCobot cell;
            Program program;
            ...
            internal CobotScriptPostProcessor(RobotCellCobot robotCell, Program program)
            {
                this.cell = robotCell;
            ...
            List<string> Program()
            {
                ...
                foreach (var cellTarget in program.Targets)
                {
                    ...
                    if (currentTool == null || target.Tool != currentTool)
                    {
                        ...
                        //string upload = "  remote_server = \"192.168.1.10\"\n  remote_port = 30000\n  ret = socket_open(remote_server, remote_port, \"socket_1\")\n  while ret != True:\n    ret = socket_open(remote_server, remote_port, \"socket_1\")\n  end";
                        //code.Add(upload);
                    }
                    ...
                    if (programTarget.IsJointTarget || (programTarget.IsJointMotion && programTarget.ForcedConfiguration))
                    {
                        ...
                        moveText = $"  movej([{joints[0]:0.####}, {joints[1]:0.####}, {joints[2]:0.####}, {joints[3]:0.####}, {joints[4]:0.####}, {joints[5]:0.####}],{external} a={axisAccel:0.####}, {speed}, r={zoneDistance})";
                    }
                    else
                    {
                            ...
                            case Motions.Joint:
                                {
                                    ...
                                    moveText = $"  movej(p[{axisAngle[0]:0.#####}, {axisAngle[1]:0.#####}, {axisAngle[2]:0.#####}, {axisAngle[3]:0.#####}, {axisAngle[4]:0.#####}, {axisAngle[5]:0.#####}],{external} a={axisAccel:0.#####},{speed},r={zoneDistance})";
                                    break;
                                }
                            ...
                            case Motions.Linear:
                                {
                                    ...
                                    {
                                        case "DIS":
                                            moveText = $"  movel(p[{axisAngle[0]:0.#####}, {axisAngle[1]:0.#####}, {axisAngle[2]:0.#####}, {axisAngle[3]:0.#####}, {axisAngle[4]:0.#####}, {axisAngle[5]:0.#####}],{external} a={linearAccel:0.#####},{speed},r={zoneDistance})";
                                            break;
                                        case "VEL":
                                            moveText = $"  movep(p[{axisAngle[0]:0.#####}, {axisAngle[1]:0.#####}, {axisAngle[2]:0.#####}, {axisAngle[3]:0.#####}, {axisAngle[4]:0.#####}, {axisAngle[5]:0.#####}],{external} a={linearAccel:0.#####},{speed},r={zoneDistance})";
                                            break;
                                    }
                                    break;
                                }
