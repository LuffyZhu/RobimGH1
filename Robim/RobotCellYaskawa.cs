...
namespace Robim
{
    public class RobotCellYaskawa : RobotCell
    {
        public static bool oneFcode { get; set; }
        internal RobotCellYaskawa(string name, List<MechanicalGroup> mechanicalGroup, IO io, Plane basePlane, Mesh environment, RobimFormSystem robimFormSystem) : base(name, Manufacturers.Yaskawa, mechanicalGroup, io, basePlane, environment, robimFormSystem) { }
        ...
        {
            RobotCellYaskawa cell;
            Program program;
            internal List<List<List<string>>> Code { get; }

            internal KRLPostProcessor(RobotCellYaskawa robotCell, Program program)
            {
                this.cell = robotCell;
                ...
                for (int i = 0; i < cell.MechanicalGroups.Count; i++)
                {
                    if (oneFcode)
                    {
                        var groupCode = new List<List<string>> { MainFile(0, i), DatFile(i) };
                    ...
                    {
                        var groupCode = new List<List<string>> { MainFile3Files(i), DatFile(i) };
                        for (int j = 0; j < program.MultiFileIndices.Count; j++)
                            groupCode.Add(SrcFile(j, i));
                ...
                for (int j = start; j < end; j++)
                {
                    ...
                    if (currentFrame == null || target.Frame != currentFrame)
                    {
                        if (target.Frame.IsCoupled)
                        {
                            ...
                            code.Add($"$BASE = EK(MACHINE_DEF[{mech}].ROOT, MACHINE_DEF[{mech}].MECH_TYPE, {a})");
                            code.Add($"$ACT_BASE=22");
